...
const WalletModal: FC<ModalProps> = (props) => {
  const { disclosure, itemInfo, permit } = props;
  ...
  const { approveLoading, onApprove, onButtonClick, errorMessage, loading, beReadyForBuyTicket } = useBuyTicket(permit);
  ...
  useEffect(() => {
    if (isAvailable) {
      isOrigin ? setBtnStsList(['disabled', undefined]) : setBtnStsList([undefined, 'wait']);
    } else {
  ...
  const toApprove = async () => {
    ...
    const approveRes = await onApprove(itemInfo!, permit);
        ...
        {(onClose) => (
          <>
            <ModalHeader className="flex justify-between relative">
              <S3Image
                onClick={onClose}
                src="/astrark/shop/btn_close.png"
                className="absolute top-[-1.3rem] right-[-1.625rem] w-[6.9375rem] h-[4.75rem] object-contain"
              ></S3Image>
            </ModalHeader>
            <ModalBody className="px-20 py-10">
              <div className="px-3 flex flex-col gap-3 text-xl">
                ...
                <div>
                  ...
                  <span className="text-[#a6c5ed] ml-1">
                    {balance === null ? '—' : `${balance} ${itemInfo?.symbol}`}
                  </span>
                </div>
                <div>
                  Payment Amount：
                  <span className="text-[#f33f3f] ml-1">
                    {itemInfo?.product_token_price_with_discount} {itemInfo?.symbol}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-8 mb-1 py-2 px-3 bg-[rgba(0,0,0,.4)] text-sm">
                {errorMessage ? (
                  ...
                  isOrigin ? (
                    'Please click Purchase button to continue your transaction in your wallet extension'
                  ) : (
              ...
              <div className="flex">
                {!isOrigin && (
                  <>
                    <PayButton btnStatus={btnStsList[0]} index={1} isLoading={approveLoading} onClick={toApprove}>
                      Approve
                    </PayButton>
                ...
                <PayButton
                  className={cn([btnStsList[1] === undefined && 'ml-[-0.5rem]'])}
