...
export class ConnectWalletQuest extends QuestBase {
  ...
  async reClaimReward(userId: string): Promise<claimRewardResult> {
    ...
    // 刷新钱包资产
    const refreshResult = await this.refreshUserWalletMetric(
      userId,
      this.user_wallet_addr,
      metrics.wallet_asset_usd_value,
    );
  ...
  async saveUserIncreasedReward(
    userId: string,
    rewardDelta: number,
    historyReward: IUserMoonBeamAudit,
    assetId: string,
  ...
  async refreshUserWalletMetric(
    userId: string,
    wallet: string,
    historyTotalValue: number = -1,
  ): Promise<refreshUserWalletMetricResult> {
    // 刷新用户资产的NFT价值
    const userNFT = await this.refreshUserWalletNFT(userId, wallet);
    logger.debug(
      `got user ${userId} wallet ${wallet} nft count ${userNFT.nfts.length} & value ${userNFT.nft_usd_value}.`,
    );
    // 检查用户资产的token价值
