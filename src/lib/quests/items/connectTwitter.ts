...
// 校验用户是否绑定了twitter
export async function verifyConnectTwitterQuest(userId: string, quest: IQuest): Promise<verifyQuestResult> {
    const twitter = await queryUserTwitterAuthorization(userId);
    return {claimable: !!twitter, auth_token: !!twitter ? twitter.token : null};
}