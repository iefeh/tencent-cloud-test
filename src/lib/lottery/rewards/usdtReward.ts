...
export class USDTReward extends LotteryRewardBase {
  ...
  async checkRewardDrawable(userId: string, drawResults?: IUserLotteryRewardItem[]): Promise<boolean> {
    const userDrawHistory = await UserLotteryDrawHistory.findOne({ user_id: userId, lottery_pool_id: this.lotteryPoolId, "rewards.reward_type": LotteryRewardType.USDT });
    if (userDrawHistory) {
      return false;
    }
    if (drawResults) {
