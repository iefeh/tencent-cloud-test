...
router
  .use(errorInterceptor(defaultErrorResponse), mustAuthInterceptor, timeoutInterceptor(defaultErrorResponse, 15000))
  .post(async (req, res) => {
...
async function checkClaimCampaignPrerequisite(req: any, res: any): Promise<ICampaign | null> {
  ...
  const campaign = await Campaign.findOne(
    { id: campaign_id, active: true, deleted_time: null },
    {
      _id: 0,
      description: 0,
    },
  ...
  if (!questAchievements) {
    ...
    const leastXTasks: string[] = tasks
      .filter((t: any) => t.finish_type == CampainTaskFinishType.Least)
      .map((t: any) => t.id);
...
async function claimCampaignRewards(userId: string, campaign: ICampaign): Promise<number> {
  ...
  //如果奖励中的badge被标注为立即领取，则立即下发badge
  ...
  await doTransaction(async (session) => {
    ...
    if (userBattlepass) {
      ...
      await UserBattlePassSeasons.updateOne(
        { user_id: userId, battlepass_season_id: userBattlepass.battlepass_season_id },
        {
...
// 构建用户活动的加成MB奖励
async function constructUserAcceleratedMbReward(
  userId: string,
  campaign: ICampaign,
  baseMbReward: number,
): Promise<any[]> {
  ...
  for (let accelerator of accelerators) {
    switch (accelerator.type) {
      case RewardAcceleratorType.NFTHolder:
        ...
        logger.info(
          `User ${userId} accelerated ${holderReward.bonus_moon_beam} MB from campaign ${campaign.id} accelerator ${accelerator.id}.`,
        );
      ...
      case RewardAcceleratorType.BadgeHolder:
        ...
        const userBadge = await UserBadges.findOne({
          user_id: userId,
          badge_id: badgeHolderAccelerator.accelerator.properties.badge_id,
        });
        //若用户未持有此徽章，则继续
        if (!userBadge) {
          continue;
        ...
        logger.info(
          `User ${userId} accelerated ${badgeHolderReward.bonus_moon_beam} MB from campaign ${campaign.id} accelerator ${accelerator.id}.`,
        );
...
async function constructCDKNotificatoin(userId: string, campaign: ICampaign) {
  // 查询奖励中的CDK通知
  const cdkNotificationIds: string[] = campaign.rewards
    .filter((r) => r.type === CampaignRewardType.CDK)
    .map((r) => r.cdk_notification_id);
