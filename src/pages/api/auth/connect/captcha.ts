...
router.use(mustAuthInterceptor).get(async (req, res) => {
    ...
    if (email) {
        ...
        let other = await User.findOne({ email: email, deleted_time: null }, { _id: 0, email: 1 });
        if (!other) {
            other = await User.findOne({ email: String(email).toLowerCase(), deleted_time: null }, { _id: 0, email: 1 });
        }
        ...
        // 检查目标是否存在绑定cd
        let canReconnectAt = await redis.get(`reconnect_cd:${AuthorizationType.Email}:${String(email).toLowerCase()}`);
        if (canReconnectAt) {
