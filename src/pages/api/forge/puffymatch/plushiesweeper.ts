...
router.use(errorInterceptor()).post(async (req, res) => {
  ...
  try {
    ...
    const endDate = new Date(end_time + 86400000).toISOString().slice(0, 10);
    const userId = await queryUserId(email, twitter, '', '');
    if (!userId) {
      return res.json(response.success({ error: response.notFound() }));
    }

    const sql = `SELECT COUNT(*)>=1,COUNT(*) ,1 from ta.v_event_14 WHERE ("$part_date" BETWEEN '${startDate}' AND '${endDate}') AND "#event_name" = 'use_item' AND "item_id" = 'plushie_sweeper' AND "account" = '${userId}'`;
    ...
    await upsertCallbackTaskOverview(userId, CallbackTaskType.FORGE_MATCH_PLUSHIE_SWEEPER, achieved);
