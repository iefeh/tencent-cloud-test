...
router.use(errorInterceptor()).post(async (req, res) => {
  ...
  try {
    ...
    const sql = `SELECT COUNT(*)>=1,COUNT(*) ,1 from ta.v_event_14 WHERE ("$part_date" BETWEEN '${startDate}' AND '${endDate}') AND "#event_name" = 'use_item' AND "item_id" = 'time_booster' AND "account" = '${userId}'`;
    ...
    await upsertCallbackTaskOverview(userId, CallbackTaskType.FORGE_MATCH_TIME_BOOSTER, achieved);
