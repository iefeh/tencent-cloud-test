...
router.use(errorInterceptor(), mustAuthInterceptor).get(async (req, res) => {
  ...
  const result = await getUserTasksCategoryInfo(userId, id as string);
  res.json(
    response.success({
...
function dealResult(item: any) {
  if (!item) return null;

  let achieveCount = 0;
  let isNew = false;
  for (let q of item.quests) {
    if (q.achieve_count) {
      achieveCount++;
  ...
  delete item.quests; //删除任务的具体信息。
  item.achieve_count = achieveCount;
  item.is_new = isNew;
  return item;
}

//查询用户任务情况总览，先通过查询分类表获取所有的分类，再分别vlookup quests表和quest_achievements表，获得对应的任务数量和完成情况。
export async function getUserTasksCategoryInfo(userId: string, id: string): Promise<any> {
              ...
              $and: [
                {
                  $or: [
                    ...
                    { end_time: { $gte: currentSeason.start_time, $lte: currentSeason.end_time } },
                    { start_time: { $lte: currentSeason.start_time }, end_time: { $gte: currentSeason.end_time } },
                  ],
                },
              ],
                          ...
                          $or: [
                            {
                              $eq: [
                                '$quest_id',
                                { $concat: [{ $toString: '$$quest_id' }, `,${format(Date.now(), 'yyyy-MM-dd')}`] },
                              ],
                            },
