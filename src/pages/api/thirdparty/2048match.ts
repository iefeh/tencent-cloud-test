...
router.use(errorInterceptor()).post(async (req, res) => {
  ...
  try {
    ...
    const sql = `SELECT  count("#event_name") >= 10,count("#event_name"),10 FROM  ta.v_event_8 WHERE ("$part_date" BETWEEN '${startDate}' AND '${endDate}') AND "#event_name" = 'game_end' AND "account" = '${userId}'`;
    ...
    await upsertCallbackTaskOverview(userId, CallbackTaskType.PUFFY_2048_MATCH_PLAY, achieved);
