...
async function redeemPremiumPassReward(userId: string, session: any, reward: any) {
  ...
  await UserBattlePassSeasons.updateOne({ user_id: userId, battlepass_season_id: reward.season_id }, { premium_type: BattlePassRequirementType.CDK, premium_source: 'CDK Premium Pass', is_premium: true, updated_time: Date.now() }, { session: session });
}
